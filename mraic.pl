...
$phyml_stat_file_ending2  = '_phyml_stat.txt'; # hack for macosx version of phyml
...
sub check_pers_files {
    ...
    my $stat_file2    = "$infile_name" . "$phyml_stat_file_ending2"; # hack for macosx version of phyml
...
sub run {
    ...
    $phyml_stat_txt_file2 = "$infilename" .  "$phyml_stat_file_ending2"; # hack for macosx version of phyml 
    ...
    for ($i=0; $i<$nmodels; $i++) {
        ...
        ##############################################################
        # error here. the first glob above succeeds! shouldn't!
        open (STATFILE, '<', $statFileName) or die "\a\nERROR!\n\ncannot open $statFileName for reading: $!";
            $tmpLnL = 999.999;
            while (<STATFILE>) {
                chomp;
        ...
        open (TREEFILE, '<', $treeFileName) or die "\a\nERROR!\n\nCannot open $treeFileName for reading: $!";
        $tmpTree = "";
            while (<TREEFILE>) {
                chomp($tmpTree = $_);
            }
...
#
sub getDimensions {
	open (INFILE,"< $infilename") or die "\a\nERROR!\n\nCannot open $infilename for reading: $!";
